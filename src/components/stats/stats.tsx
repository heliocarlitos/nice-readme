...
export function Stats() {
  ...
  useEffect(() => {
    if (!username.trim()) {
      setImageUrl("")
      setMarkdown("")
      setLoading(false) // ← garantir que loading desaparece
      return
    }
    ...
    const timer = setTimeout(() => {
      ...
      setImageUrl(url)
      setMarkdown(`[![${decodeURIComponent(customTitle)}](${url})](https://github.com/${username})`)
      const img = new Image()
  ...
  }, [username, buildImageUrl, customTitle])
          ...
          <div className="content">
            ...
            <div className="box">
              ...
              <div className="input-box">
                ...
                <input type="number" id="card_width" min="270" value={cardWidth} onChange={e => setCardWidth(parseInt(e.target.value) || 499)} />
              </div>
            </div>
              ...
              {imageUrl && !loading && (
                <>
                  <figure>
                    <img
                      src={imageUrl}
                      alt="Pré-visualização das estatísticas do GitHub"
                      width={cardWidth}
                      height="auto"
                      onError={e => {
                        ;(e.target as HTMLImageElement).alt = "Erro ao carregar. Verifique se o utilizador existe."
                      }}
                    />
                  </figure>
                  <CodeCard code={markdown} lang="Markdown" />
                  <CodeCard code={`<img src="${imageUrl}" alt="GitHub Stats de ${username}" width="${cardWidth}" height="auto" loading="lazy" />`} lang="HTML" />
                </>
              )}
